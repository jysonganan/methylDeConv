...
geoMat <- getGEO("GSE116298")
pD <- pData(geoMat[[1]])
rgSet<- read.metharray.exp("GSE116298/idat",force = TRUE)
...
ggplot(df) + geom_boxplot(aes(series ,value,color=group)) +
  xlab('cell types')+
  ylab('proportions') +
  ggtitle("GSE116298-GBM_Houseman-onevsAllttest")
dev.off()
...
ggplot(df) + geom_boxplot(aes(series ,value,color=group)) +
  xlab('cell types')+
  ylab('proportions') +
  ggtitle("GSE116298-GBM_Houseman-glmnetpreselect")
dev.off()
...
group <- pD[,"tissue:ch1"]
gender_dat_blood <- matrix(NA, 47, 9)
samples <- rownames(Houseman_res_epicEpithelial)
rownames(gender_dat_blood) <- samples
gender_dat_blood[,1:8] <- Houseman_res_epicEpithelial
gender_dat_blood[,9] <- group
...
ggplot(df) + geom_boxplot(aes(series ,value,color=group)) +
  xlab('cell types')+
  ylab('proportions') +
  ggtitle("GSE116298-GBM_Houseman-onevsAllttest (EPICEpithelial)")
dev.off()
...
library(GEOquery)
library(minfi)
# getGEOSuppFiles("GSE112308")
# untar("GSE112308/GSE112308_RAW.tar", exdir = "GSE112308/idat")
# head(list.files("GSE112308/idat", pattern = "idat"))
# 
# idatFiles <- list.files("GSE112308/idat", pattern = "idat.gz$", full = TRUE)
# sapply(idatFiles, gunzip, overwrite = TRUE)

geoMat <- getGEO("GSE112308")
pD <- pData(geoMat[[1]])
rgSet<- read.metharray.exp("GSE112308/idat",force = TRUE)
betaMat <- getBeta(preprocessNoob(rgSet, dyeMethod = "single"))
...
gender_dat_blood <- matrix(NA, 30, 7)
samples <- rownames(Houseman_res_epic)
rownames(gender_dat_blood) <- samples
gender_dat_blood[,1:6] <- Houseman_res_epic
gender_dat_blood[,7] <- group
...
ggplot(df) + geom_boxplot(aes(series ,value,color=group)) +
  xlab('cell types')+
  ylab('proportions') +
  ggtitle("GSE112308-Melanoma_Houseman-onevsAllttest")
dev.off()
...
ggplot(df) + geom_boxplot(aes(series ,value,color=group)) +
  xlab('cell types')+
  ylab('proportions') +
  ggtitle("GSE112308-Melanoma_Houseman-glmnetpreselect")
dev.off()
...
group <- pD[,"condition:ch1"]
gender_dat_blood <- matrix(NA, 30, 9)
samples <- rownames(Houseman_res_epicEpithelial)
rownames(gender_dat_blood) <- samples
gender_dat_blood[,1:8] <- Houseman_res_epicEpithelial
gender_dat_blood[,9] <- group
