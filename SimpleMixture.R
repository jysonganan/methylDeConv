...
corr <- rep(NA, 600)
for (i in 1:600){
  corr[i] <- cor(proportions_sim[i,c("Bcell", "CD4T","CD8T","Mono","Neu","NK")], RPC_res[i,], method = "spearman")
}


corr <- rep(NA, 6)
for (i in 1:6){
  corr[i] <- cor(proportions_sim[,c("Bcell", "CD4T","CD8T","Mono","Neu","NK")][,i], RPC_res[,i], method = "spearman")
}
